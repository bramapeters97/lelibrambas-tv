...
- The native Privacy Manifest declares no tracking, tracking domains, collected-data declarations, or required-reason API categories because the app uses first-party network/Keychain APIs and no tracking SDK. Privacy Manifest contents do **not** replace App Store Connect server-side data disclosures.
