...
`bootstrap-mac.sh` creates ignored local signing configuration only when absent; it never overwrites populated local configuration. The Debug simulator build uses synthetic fixture content only when launched with test-only arguments. Release presents the native 5.8-second LeliBrambas+ ident with the bundled web-viewer jingle, then the three local profiles, and then Home on cold launch and after every genuine background-to-active return. A background return clears the local profile selection; a transient inactive state does not restart the sequence. It loads `../../data/media_catalog.json`, `../../artwork/`, and the official root studio image from the app bundle. No gateway, account, activation, remote session, or token configuration is required.
