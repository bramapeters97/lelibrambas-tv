...
`bootstrap-mac.sh` creates ignored local signing configuration only when absent; it never overwrites populated local configuration. The Debug simulator build uses synthetic fixture content only when launched with test-only arguments. Release presents one second of the native ident background, runs the unchanged branded logo/jingle animation, then holds the completed logo for three seconds with a gold loader before showing the three local profiles. This sequence runs on cold launch and after every genuine background-to-active return. A background return clears the local profile selection; a transient inactive state does not restart the sequence. Each production start requests and strictly validates the public movies API. Poster artwork is loaded directly from each item’s absolute HTTPS `poster_url`; failed artwork uses the existing code-rendered placeholder. No catalogue JSON, poster PNG, alternate catalogue endpoint, or offline catalogue fallback is bundled. Playback progress is stored locally per profile and film so details can offer Resume or Restart; it is never uploaded. No gateway, account, activation, remote session, or token configuration is required.
