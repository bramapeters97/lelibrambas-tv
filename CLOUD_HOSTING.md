...
For the GitHub-connected `lelibrambas-tv` Worker that serves the browser viewer, configure the
production branch as `main`, root directory as `/`, build command as `corepack yarn build`, deploy
command as `npx wrangler deploy`, and version command as `npx wrangler versions upload`. Set the
build variables `YARN_VERSION=1.22.22` and `NODE_VERSION=24.19.0`. The root `wrangler.jsonc` is the
source of truth for the Worker name, compatibility date, generated static asset directory, and SPA
fallback. `apps/tv/index.html` is Vite's entry point and becomes `apps/tv/dist/index.html`.

The custom domain `lelibrambas.com` must be attached to that same Worker in Cloudflare. DNS
