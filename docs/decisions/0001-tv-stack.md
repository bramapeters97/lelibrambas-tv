# ADR 0001: Expo SDK 57 television and phone stack
...
| `react-native`                              | `npm:react-native-tvos@0.86.2-0` | Native Android TV/tvOS fork matching RN 0.86.2.                                 |
| `react-native-web`                          | `0.21.2`                         | Phone Safari preview; the rich viewer remains direct React DOM.                 |
| `expo-video`                                | `57.0.2`                         | Bundled native TV/phone playback; also rendered by the phone web preview.       |
| `expo-image`                                | `57.0.3`                         | TV-supported image package, compatibility-pinned but not yet used by the shell. |
| `react-native-reanimated`                   | `4.5.1`                          | SDK 57 TV-supported animation package; not yet used by the shell.               |
| `react-native-worklets`                     | `0.10.1`                         | Required Reanimated generation.                                                 |
| `@react-native-async-storage/async-storage` | `2.2.0`                          | TV-supported device persistence dependency.                                     |
| `@react-native-tvos/config-tv`              | `0.1.6`                          | CNG target switch for TV versus phone generation.                               |
...
Expo's current compatibility table maps SDK 57 to React Native 0.86, React 19.2.3, React Native Web 0.21.0, React Native TV `0.86-stable`, and Node 22.13.x. React Native TV publishes the matching 0.86 line as the `react-native` alias; exact `0.86.2-0` avoids an unreviewed floating stable tag.
...
The Expo TV guide lists AsyncStorage, Image, Reanimated, and Video as TV-capable and explains the React Native TV alias, config plugin, clean prebuild, Android TV/Apple TV commands, and separate tvOS provisioning profile. `expo-video`'s SDK 57 page explicitly lists Android, tvOS, and web and recommends `~57.0.2`.

## Rendering and navigation

`apps/tv/App.tsx` dispatches on `Platform.isTV`. Its TV branch uses `Pressable`,
...
TV pass does not establish parity with another surface; no such parity is currently claimed.

No Expo Router or React Navigation dependency is installed. Explicit state is adequate for the
current native flows. Expo lists React Navigation as compatible with TV, so it is the preferred
...
`apps/tv/app.config.ts` derives the target from `EXPO_TV`: `1`/`true` produces the TV plugin target
and landscape orientation; other values produce the phone target and default orientation. The root
`tv:prebuild` and `android:tv` commands reach workspace scripts that explicitly set `EXPO_TV=1`.
The mobile development, prebuild, and iOS scripts explicitly set `EXPO_TV=0`. CNG output must be
regenerated cleanly when changing targets.

## Platform constraints

- Expo SDK 57 compiles/targets the Android 16/API 36 toolchain; install Platform/Build Tools 36 and an API 36 TV image where available. Expo permits TV images API 31+.
- React Native TV 0.86 supports Android API 24+. The application still needs real TV emulator/device QA.
...
- Windows local EAS Build is unsupported. Use `expo run:android` for local Windows development; tvOS local build remains Mac-only.
...
stable branch, update exact pins/lockfile, regenerate native projects, and rerun phone web, native
iPhone, Android TV, and tvOS checks. Test playback features on each target rather than inferring
